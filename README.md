...
2. Иначе сравнить K с ключом X корневого узла root:
	...
	c. Если K=X, то необходимо рассмотреть три случая.
		...
		i. Если обоих детей нет, то удаляем текущий узел и обнуляем ссылку на него 
		...
		iii. Если оба ребёнка присутствуют, то:
			...
			1. найдём узел, являющийся самым левым узлом правого 
			поддерева с корневым узлом Right(root);
