...
Сохранить d-кучу можно в обычном массиве (сначала корень, потом ݀ его потомков, затем ݀ потомков первого потомка корня и т.д., см. Рис. 2), но с точки зрения программной реализации это не совсем удобно, т.к. при выполнении некоторых операций с приоритетными очередями может потребоваться перепаковка элементов.
...
=======
```}
>>>>>>> c04d20f65aa6a11a7613f1c065a866cae6801724
...
**Операция окучивания**
Данная операция позволяет организовать из набора ключей d-кучу. Кучеобразность согласно определению предполагает, что веса потомков каждого узла больше веса этого узла. Оказывается, что если деревья с корнями, имеющими номера n − 1, n − 2, ... , n - i окучены, применяя операцию погружения для узла n − i − 1, дерево с корнем n − i − 1 тоже оказывается окученным.
...
=======
   левым поддеревом «бывшего» корневого узла.![start](/img/рис5.PNG) Рис.3
![start](/img/рис6.PNG) Рис.4
>>>>>>> c04d20f65aa6a11a7613f1c065a866cae6801724
...
####Структура данных "Упорядоченные таблицы"####
	...
	Упорядоченные таблицы – это таблицы, в которых записи располагаются в порядкевозрастания (или убывания) ключей.
